...
// ─── FETCH & RENDER ───────────────────────────────────────────────────────────
const WX_URL           = 'https://api.open-meteo.com/v1/forecast?latitude=33.7701&longitude=-118.1937&current=temperature_2m,apparent_temperature,relative_humidity_2m,wind_speed_10m,weather_code,uv_index&daily=weather_code,temperature_2m_max,temperature_2m_min&temperature_unit=fahrenheit&wind_speed_unit=mph&timezone=America%2FLos_Angeles&forecast_days=5';
const WX_INTERVAL_MS   = 10 * 60 * 1000;
const WX_FAIL_DELAYS   = [30000, 60000, 120000];
...
async function loadWeather() {
  try {
    const r = await fetchWithTimeout(WX_URL, 15000);
    ...
    for (let i = 1; i <= 4; i++) {
      ...
      row.className = 'fc';
      row.innerHTML = `
        <div class="fc-n">${DAYS[dt.getDay()]}</div>
        <div class="fc-icon">${getWeatherIcon(code)}</div>
        <div class="fc-cond">${WX_SHORT[code] || '—'}</div>
        <div class="fc-temps"><span class="fc-h">${hi}°</span><span class="fc-lo">${lo}°</span></div>`;
      fc.appendChild(row);
    }

    wxRetryCount = 0;
    scheduleNextWeather(WX_INTERVAL_MS);
  } catch(e) {
    document.getElementById('wx-condition').textContent = 'Unavailable';
    const delay = WX_FAIL_DELAYS[Math.min(wxRetryCount, WX_FAIL_DELAYS.length - 1)];
    wxRetryCount++;
